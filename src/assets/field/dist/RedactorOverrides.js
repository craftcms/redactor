...
imageResizeClass.prototype.init = function (app) {
  ...
  this.inspector = app.inspector; // init

  this.$target = this.toolbar.isTarget() ? this.toolbar.getTargetElement() : this.$body; // Change the target according to LP

  var attachLivePreview = function attachLivePreview() {
    _this.hide();

    var $editor = $('.lp-editor');

    if ($editor.length) {
      _this.$target = $R.dom($editor[0]);
    }
  };

  var detachLivePreview = function detachLivePreview() {
    _this.hide();

    _this.$target = _this.toolbar.isTarget() ? _this.toolbar.getTargetElement() : _this.$body;
  };
...
toolbarFixedClass.prototype._init = function () {
  ...
  var attachLivePreview = function attachLivePreview() {
    ...
    if ($editor.length) {
      _this2.livePreview = true;
      _this2.$previousFixedTarget = _this2.$fixedTarget;
      $editor = $R.dom($editor[0]);
      $editor.on('scroll.redactor-toolbar-' + _this2.uuid, _this2._doFixed.bind(_this2));
      _this2.$fixedTarget = $editor;
    }
  };

  var detachLivePreview = function detachLivePreview() {
    _this2.livePreview = false;
    _this2.$fixedTarget = _this2.$previousFixedTarget;
    _this2.$previousFixedTarget = null;
  };
...
toolbarFixedClass.prototype._doFixed = function () {
  ...
  pinDistance = $editor.scrollTop() + headerBuffer; // Figure out when to pin the toolbar
...
inputCleanerService.prototype.input = function (html, paragraphize, started) {
  ...
  html = html.replace(/¤t/gi, '&current'); // store

  var storedComments = [];
  html = this.storeComments(html, storedComments); // pre/code
  ...
  }.bind(this)); // get wrapper html

  html = this.utils.getWrapperHtml($wrapper); // converting entity

  html = html.replace(/\$/g, '&#36;');
  html = html.replace(/&amp;/g, '&'); // convert to figure

  var converter = $R.create('cleaner.figure', this.app);
  html = converter.convert(html, this.convertRules); // store components
  ...
  html = this.opts.removeScript ? this._removeScriptTag(html) : this._replaceScriptTag(html); //html = (this.opts.removeScript) ? this._removeScriptTag(html) : html;

  html = this.opts.removeComments ? this.removeComments(html) : html;
  html = this._isSpacedEmpty(html) ? this.opts.emptyHtml : html; // restore components
...
inputCleanerService.prototype.output = function (html, removeMarkers) {
  ...
  html = html.replace(/&#36;/g, '$'); // empty

  if (this._isSpacedEmpty(html)) return '';
  if (this._isParagraphEmpty(html)) return '';
  html = this.removeServiceTagsAndAttrs(html, removeMarkers); // store components
  ...
  html = this.opts.removeNewLines ? this.cleanNewLines(html) : html; // restore components

  html = this.restoreComponents(html); // convert to figure

  var converter = $R.create('cleaner.figure', this.app);
  html = converter.unconvert(html, this.unconvertRules); // final clean up

  html = this.removeEmptyAttributes(html, ['style', 'class', 'rel', 'title']);
  html = this.cleanSpacesInPre(html);
  html = this.tidy(html); // converting entity
...
contextBarClass.prototype.init = function (app) {
  ...
  var attachLivePreview = function attachLivePreview() {
    ...
    if ($target.length) {
      $(_this3.$contextbar.get()).appendTo($target);
      _this3.livePreview = true;
    }
  };

  var detachLivePreview = function detachLivePreview() {
    var $target = _this3.toolbar.getTargetElement();

    if ($target.length) {
      $(_this3.$contextbar.get(0)).appendTo($target.get(0));
    }

    _this3.livePreview = false;
  };
...
toolbarService.prototype.addButton = function (name, btnObj, position, $el, start) {
  ...
  if (this.is()) {
    if (position === 'first') this.$toolbar.prepend($button);else if (position === 'after') $el.after($button);else if (position === 'before') $el.before($button);else {
      ...
      if (start !== true && index !== -1) {
        if (index === 0) {
          ...
          var $btns = this._findButtons(); // If this was an added button, it might not exist in the options with how things are, so make sure to stay inside bounds
