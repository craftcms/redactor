...
Craft.Redactor.PluginBase = {
  ...
  overrideButton: function overrideButton(replaceButton) {
    ...
    var placeholder = this.app.toolbar.addButtonAfter(replaceButton, placeholderKey, {
      title: this.title
    });
  ...
  addButton: function addButton(buttonName, index) {
  ...
  setElementSiteId: function setElementSiteId(siteId) {
    this.elementSiteId = siteId;
  },
  registerCmdS: function registerCmdS(saveCb, closeCb) {
    // Add a new layer
    Garnish.shortcutManager.addLayer(); // Trigger the save callback on Cmd+S
  ...
  saveSelection: function saveSelection(app) {
  ...
  restoreSelection: function restoreSelection(app) {
