...
gulp.task('redactor-clean', () => {
  return del([`${redactorSrcPath}/**/*.min.*`, `${pluginsSrcPath}/**/*.min.*`]);
});

gulp.task('redactor-js', ['redactor-clean'], () => {
  return gulp
    .src(`${redactorSrcPath}/**/*.js`)
...
gulp.task('redactor-css', ['redactor-clean'], () => {
  return gulp
    .src(redactorSrcPath + '/redactor.css')
    .pipe(sourcemaps.init())
    .pipe(autoprefixer())
    .pipe(cleanCSS())
    .pipe(sourcemaps.write('.'))
